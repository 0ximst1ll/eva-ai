...
`src/core/session-store.ts` 提供当前 workspace 级 JSONL store 边界，负责 workspace session 目录、session 文件路径、manifest、session log 读写、append 和文件枚举。`src/core/session-entry-store.ts` 提供当前单 session entry store 边界，负责 entry tree、path entries、active entry id、entry path traversal 和 entry tree view。`src/core/session-model.ts` 提供当前最小 session 语义状态容器，收敛 metadata、lineage、schema format、entry store 和从 active entry path 派生出的 active state cache，并承接 append message / usage / internal / compaction、branch active leaf 切换、fork/clone 的 target session model 构造，以及 create/reset 的 initial model 构造。`SessionManager` 仍保留 public API 和 session lifecycle 编排，但不再用多组 per-session Map 分散保存运行期状态；append 和 branch 路径由 `SessionModel` 返回 durable entry，fork/clone 路径由 `forkSessionModel()` 返回 target model、path entries 和 lineage，create/reset 路径由 `createInitialSessionModel()` 返回 model、initial entry 和 lineage，`SessionManager` 只负责 jsonl 持久化和 manifest 更新。后续可在这些边界之上继续拆出 workspace `SessionRepo` 和更完整的语义层 `Session`。
...
- load/import 会通过统一 parsed session application 边界恢复 metadata、entry tree、path entries 和 active state；缺少 entry metadata 的旧 flat JSONL 不再加载为有效 session。
