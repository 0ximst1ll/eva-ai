...
当前内置工具 renderer 已开始按 `pi-mono` 风格做 tool-specific collapsed preview：`read_file` 展示前 10 行，`grep_files` 展示前 15 行，`find_files` / `list_files` 展示前 20 行，`bash` 展示 tail 5 行。TUI/CLI 展示层可用 tool definition 和 tool args 重新渲染结果；TUI 已支持通过 `Ctrl-T` 切换最近工具结果的 collapsed/expanded 展示。bash streaming accumulator 尚未完成。
