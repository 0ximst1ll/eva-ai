...
- `parseSessionLog()` 会记录 invalid JSON、unsupported schema、entry before start、missing entry metadata、unknown entry type、active leaf missing 和 broken parent chain 等 structured diagnostics；`loadSession()` 保持 boolean API，同时把失败原因记录到 `SessionManager.getDiagnostics()`。
