...
- `forkSession()` 会复制指定 leaf entry path 到新 session，并写入 lineage metadata；未指定 leaf 时使用当前 active entry path；没有 active entry path 的 session 不再 fork；fork 后父子 session 的后续消息互不影响。
