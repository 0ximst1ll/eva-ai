...
- `getActiveState()` 是 `SessionManager` 的 active state 读取边界，会返回 copy 后的 active messages、compaction、usage 和 internal entries；append message/usage/internal/compaction 路径会以该 active state 作为当前 view，避免 abandoned branch state 泄漏。
