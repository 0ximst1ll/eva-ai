...
`src/core/session-store.ts` 提供当前 workspace 级 JSONL store 边界，负责 workspace session 目录、session 文件路径、manifest、session log 读写、append 和文件枚举。`SessionManager` 仍保留 public API 和 session 语义，但 JSONL 文件生命周期已开始从单体类中移出；后续可在该边界之上继续拆出单 session `SessionStorage`、workspace `SessionRepo` 和语义层 `Session`。
