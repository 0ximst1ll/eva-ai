...
- load/import 会通过 `createSessionModelFromParsedLog()` 恢复 metadata、entry tree、path entries 和 active state；缺少 entry metadata 的旧 flat JSONL 不再加载为有效 session。JSONL parser 当前仍保留在 `SessionManager`。
