...
- `/children`：通过 `RuntimeHost.listChildSessions()` 列出当前 session 的 direct child sessions。
- `/child [sessionId]`：通过 `RuntimeHost.switchToChildSession()` 切换到 direct child session；没有参数且只有一个 child 时直接切换，多个 child 时要求指定 session id。
...
- `branch_session`：通过 `RuntimeHost.branchSession()` 在当前 session 文件内移动 active leaf；必填 `leaf_entry_id`，response 会包含 branch operation summary。
...
- 为当前通过 `tools.skills_dir` 发现的 skills 写入 source metadata，当前 `source=config`，scope 按目录是否在 workspace 内分为 `project` 或 `user`；
- 通过 `SkillSourceCandidate` 内部管线加载 skills，按 source priority 合并同名 skill；
- 对 skills 目录缺失、非法 frontmatter、重名等情况返回非致命 diagnostics；
- 对 MCP 已配置但需要未来 extension boundary 的情况返回 warning diagnostic。

当前 skills discovery 是最小版，参考 `pi-mono` 的 resource-first 思路：skills 是 resource，不是 builtin tool。它支持配置目录内递归发现目录型 `SKILL.md`，也支持配置目录根级 `.md` skill；skill frontmatter 需要 `name` 和 `description`，可选 `disable-model-invocation`。重复 `name` 会按 source priority 合并：更高 priority 替换低 priority，同 priority 保留先出现的 source，并对被忽略项返回 warning diagnostic。当前 `runtime.services.resourceLoader.skills` 会保留 metadata/content/sourceInfo；`sourceInfo` 包含 `source`、`scope`、`configuredPath` 和 source `baseDir`，用于后续 package/extension source、merge/dedupe 和 reload diagnostics。`ContextBuilder` 会把未设置 `disable-model-invocation` 的 skill metadata 追加到 system prompt，只包含 `name`、`description` 和 `location`，不会默认注入 skill 全文。用户通过 `/skill:name` 或 `/skill name` 显式 invocation 时，`ContextBuilder` 会把匹配 skill 的完整内容作为一次性 transient `<invoked_skills>` user context 注入下一次 provider request；该内容不写入 durable session history。
...
- 将 skills metadata 追加到本次 provider request 的 system prompt；
- 将 queued skill invocations 作为一次性 transient user context 注入 provider request；
...
当前 skills metadata 注入格式：

```text
<available_skills>
<skill name="code-review" location="/workspace/skills/review/SKILL.md">
Review code changes for defects
</skill>
</available_skills>
```

当前 explicit skill invocation 注入格式：

```text
<invoked_skills>
<skill name="code-review" location="/workspace/skills/review/SKILL.md">
References are relative to /workspace/skills/review.
...
- compact 成功后会向 Agent working history 追加 transient `compaction_summary` internal marker，provider request 和 provider-visible message entries 都不会包含该 marker；
...
- `memory`：进程内 active state cache；
- `jsonl`：写入 `~/.eva-ai/sessions/<encoded-workspace>/` 下的 append-only 文件。

`src/core/session-store.ts` 提供当前 `SessionStorage` backend 边界，包含 `JsonlSessionStorage` 和 `MemorySessionStorage`，负责 session log、manifest、append、list、copy 和可选 session file path。`src/core/session-log-parser.ts` 提供当前 JSONL parser / import rewrite 边界，负责解析 session log、读取 import source session id、重写 imported session log，并返回 structured parse diagnostics。`src/core/session-entry-store.ts` 提供当前单 session entry store 边界，负责 entry tree、path entries、active entry id、entry path traversal 和 entry tree view。`src/core/session-model.ts` 提供当前最小 session 语义状态容器，收敛 metadata、lineage、schema format、entry store 和从 active entry path 派生出的 active state cache，并承接 append message / usage / internal / compaction、branch active leaf 切换、fork/clone 的 target session model 构造、create/reset 的 initial model 构造，以及 parsed log 到 model 的恢复应用。`SessionManager` 仍保留 public API 和 session lifecycle 编排，但不再用多组 per-session Map 分散保存运行期状态，也不再直接通过 `mode` 分支决定存储实现；append 和 branch 路径由 `SessionModel` 返回 durable entry，fork/clone 路径由 `forkSessionModel()` 返回 target model、path entries 和 lineage，create/reset 路径由 `createInitialSessionModel()` 返回 model、initial entry 和 lineage，load/import 路径通过 `parseSessionLog()` 与 `createSessionModelFromParsedLog()` 解析并应用 parsed log 到 model，`SessionManager` 通过 `SessionStorage` 处理 manifest 和 session log lifecycle，并通过 `getDiagnostics()` 暴露 load/import/list/latest 相关 session diagnostics。后续可在这些边界之上继续拆出 workspace `SessionRepo` 和更完整的语义层 `Session`。

存储 backend：

- `JsonlSessionStorage`：写入 `~/.eva-ai/sessions/<encoded-workspace>/` 下的 append-only 文件。
- `MemorySessionStorage`：用进程内 Map 保存同样的 session log 和 manifest，可通过显式注入在多个 `SessionManager` 实例间共享。
- `SessionManager` 构造函数仍兼容 `mode: 'memory' | 'jsonl'` shortcut；未显式传入 storage 时按 mode 创建默认 backend。

Session log 语义：

- JSONL backend 中每个 session 对应 `<sessionId>.jsonl`；
- 创建或 reset session 时写入 `session_start`；
- `session_start` 包含当前 `schemaVersion` 和 lineage metadata：`parentSessionId`、`rootSessionId`、`forkedFromMessageIndex`；
- 每条 message 都写成一个 `message` entry；
- `message`、`compaction`、`usage`、`internal`、`leaf` 和 `branch_summary` entry 都必须带有 `entryId` / `parentEntryId`，形成当前 session 文件内的 append-only parent chain；
- 旧 flat JSONL entry 没有 `entryId` / `parentEntryId` 时不再作为有效 session context 读取；
...
- `listChildSessions()` 基于 `parentSessionId` 返回指定 session 的 direct child sessions，用于 mode 层 child navigation。
- `getCompactionInfo()` 返回当前 session 最近一次 compaction metadata；如果尚未 compact，则返回 `compacted: false`。
- `getMessages()`、`getCompactionInfo()`、`getUsageInfo()` 和 `getInternalEntries()` 会通过 `getActiveState()` 从当前 active entry path 派生状态；无 entry path 的 session 不再回退 flat snapshot cache。
...
- `getLineageInfo()` 返回当前 session 的 root/parent/fork point 信息。
...
- load/import 会通过 `session-log-parser.ts` 的 `parseSessionLog()` 解析 JSONL，再通过 `createSessionModelFromParsedLog()` 恢复 metadata、entry tree、path entries 和 active state；缺少 entry metadata 的旧 flat JSONL 不再加载为有效 session。
- `parseSessionLog()` 会记录 invalid JSON、invalid entry payload、unsupported schema、entry before start、missing entry metadata、unknown entry type、active leaf missing 和 broken parent chain 等 structured diagnostics；parser 会校验各类 entry 的必要 payload 字段；`loadSession()` 保持 boolean API，同时把失败原因记录到 `SessionManager.getDiagnostics()`。
- `branchSession()` 会委托 `SessionModel.branchToEntry()` 在同一个 session 文件内移动 active entry leaf，重建当前 active state，追加 durable `leaf` entry 记录 active leaf 切换，然后追加 `branch_summary` entry；下一次 append 会以该 summary entry 作为 parent；返回值包含 path entries 数、message 数、目标 entry view 和 summary entry id。
- `forkSession()` 会委托 `forkSessionModel()` 复制指定 leaf entry path 到新 session，并写入 lineage metadata；未指定 leaf 时使用当前 active entry path；没有 active entry path 的 session 不再 fork；fork 后父子 session 的后续消息互不影响。
- `cloneSession()` 当前按 `pi-mono` 的 clone 语义复用 leaf entry path fork：复制指定 leaf entry path 到新 session，并保留 parent/root lineage。
- `exportSession()` 会通过当前 `SessionStorage.copySessionLog()` 将 session 导出为 JSONL 文件；JSONL backend 会复制原始 session 文件，memory backend 会写出内存中的 session log。
