...
- create/reset/fork 会先建立 session path entries，再创建或替换对应 `SessionModel`，通过 `applyEntryPathState()` 初始化运行期 active state cache。
