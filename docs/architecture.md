...
工具展示当前采用 `renderCall + content + durable typed details + displayContent` 最小边界。工具定义可通过 `renderCall(args)` 生成 tool call 摘要；`content` 仍是写回模型的文本；`details` 承载工具结构化信息，例如截断统计、bash exit code、full output path、行数或结果数，并会随 tool message 持久化到 session。默认 provider request 转换会剥离 durable details，只发送 provider 需要的 tool result 文本。工具定义可通过 `renderResult` 基于 details 和 `expanded/isPartial` render options 生成 `displayContent`，agent-loop 会透传给 TUI/CLI 展示和后续 compaction 使用；`/diagnostics` 不作为 tool details 的主要消费路径。
