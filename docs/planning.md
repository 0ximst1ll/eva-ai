...
- `SessionStorage` backend interface 已从 `SessionManager` 抽出，`JsonlSessionStorage` 负责 session 文件路径、manifest、session log 读写、append 和文件枚举，`MemorySessionStorage` 提供进程内 session log / manifest backend。
...
- `SessionStorage` 最小 backend interface 已完成，使 in-memory 和 JSONL 存储共享同一组 load/list/manifest/append 边界；后续再按真实需求拆完整 repo/storage/session 边界。
