...
- M4.x Entry Tree First 后续：TUI `/entries` entry selector 最小 UX 已完成；下一步可以评估 session-level child navigation，或继续减少 active state cache 的写入职责。

## 下一步

- 评估是否需要补 session-level child navigation，或继续减少 active state cache 的写入职责。
