...
当前 M4 已完成最小闭环：`SessionManager` 支持向后兼容的 lineage metadata、`forkSession()`、`cloneSession()`、`exportSession()`、`importSession()`、`listSessionTree()` 和旧 JSONL root fallback；RuntimeHost 暴露 `forkSession()`、`cloneSession()`、`exportSession()`、`importSession()` 和 `switchToParentSession()`；interactive/TUI 可通过 `/fork [id] [--entry <entryId>]` 创建当前 session 分支，也可通过 `/clone [id] [--entry <entryId>]` 按 `pi-mono` current-leaf fork 语义克隆当前 session；RPC 已支持 `fork_session` / `clone_session` 并可传入 `leaf_entry_id`；interactive/TUI 可通过 `/export [path]`、`/import <path>` 做 JSONL import/export；interactive `/sessions` 已展示 session tree，`/parent` 可切换到当前 session 的 parent session；新写入的 session entries 已带有 `entryId` / `parentEntryId`；`SessionManager.getEntryPath()` 可从 active entry leaf 回溯 path entries；`SessionManager.forkSession()` / `cloneSession()` 已优先复制指定 leaf entry path 到新 session；`SessionContextRebuilder` 已支持新 session 的 `entry_path` rebuild，并对旧 JSONL 回退 `flat_snapshot`；`SessionManager.loadSession()` 已在主加载路径中使用 active entry path 重建 active messages。
...
- RPC 已支持 `prompt`、`get_state`、`abort`、`new_session`、`resume_session`、`fork_session` 和 `clone_session`。
...
- interactive/TUI slash command 已支持 `/fork [id] [--entry <entryId>]`、`/clone [id] [--entry <entryId>]`、`/export [path]` 和 `/import <path>`。
...
- M4 后续：entry-tree-first 对齐计划已明确，指定 leaf entry fork/clone 最小步已实现；entry-level branch/navigate、更完整的 child branch navigation、跨 session parent/child entry graph 和 sidecar metadata 尚未实现。

## 下一步

- 继续 M4：进入 entry-level branch/navigate 最小边界，先支持同一个 session 文件内移动 active leaf 并从 leaf path 派生 context。
...
- 当前只支持当前 session 文件内的指定 leaf entry path fork/clone 和 session-level parent navigation；`SessionManager` 仍维护 active `Message[]` 派生状态，entry-tree-first、entry-level branch/navigate、跨 session parent/child entry graph 与完整 child branch navigation 还未实现。
