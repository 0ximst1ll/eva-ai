...
- session 管理继续完善：当前正在评估 `SessionModel` 稳定后的下一步语义拆分边界，重点是避免 `SessionManager` 继续承担过多 branch/fork/compact/context 组合职责。

## 下一步

- 保持 `SessionManager` public API 不变，继续把 branch/fork/compact/context 派生组合逻辑移出 `SessionManager` 单体类。
- 评估下一步是否拆出更明确的 session semantic operation 边界，或先补 tree navigation / branch summary pipeline。
- 后续补完整 tree navigation 交互和 branch summarization pipeline。

## 已知问题

- `SessionManager` 仍是偏大的 facade，同时负责 session lifecycle、fork/clone、import/export、compaction 和 lineage；workspace JSONL 文件 IO、单 session entry store 与最小 session model 已拆出，但尚未拆成完整 repo/storage/session 三层。
