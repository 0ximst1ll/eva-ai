...
- session 管理继续完善：当前正在评估 `SessionModel` append semantic operation 稳定后的下一步拆分边界，重点是 branch/fork/compact/context 组合职责。

## 下一步

- 保持 `SessionManager` public API 不变，继续评估 branch/fork/compact/context 派生组合逻辑的下一步拆分。
- 优先考虑把 branch active leaf 应用和 durable control entry 追加进一步收敛到 `SessionModel`。
- 后续补完整 tree navigation 交互和 branch summarization pipeline。

## 已知问题

- `SessionManager` 仍是偏大的 facade，同时负责 session lifecycle、fork/clone、import/export 和部分 branch 编排；workspace JSONL 文件 IO、单 session entry store、最小 session model 与 append semantic operation 已拆出，但尚未拆成完整 repo/storage/session 三层。
