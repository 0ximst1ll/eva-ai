...
- session 管理继续完善：下一步应在 workspace store 和 entry store 之上继续拆 session 语义层，减少 `SessionManager` 的 branch/fork/compaction 组合职责。

## 下一步

- 继续参考 pi-mono harness 的 `SessionStorage` / `SessionRepo` / `Session` 分层，拆出 session 语义对象。
- 保持 `SessionManager` public API 不变，逐步把 branch/fork/compact/context 派生组合逻辑移出单体类。
- 后续补完整 tree navigation 交互和 branch summarization pipeline。

## 已知问题

- `SessionManager` 仍是偏大的 facade，同时负责 session lifecycle、fork/clone、import/export、compaction 和 lineage；workspace JSONL 文件 IO 与单 session entry store 已拆出，但尚未拆成完整 repo/storage/session 三层。
