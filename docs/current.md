...
- `SessionManager.branchSession()` 已支持在同一个 session 文件内移动 active leaf，并把当前 active messages、compaction、usage 和 internal entries 重建为该 leaf path；下一次 append 会从该 leaf 形成新分支；返回值包含 path entries 数、message 数和目标 entry view。
...
- M4 后续：entry-tree-first 对齐计划已明确，指定 leaf entry fork/clone、entry-level branch、entry path state derivation、entry tree active path 展示和 branch operation summary 最小步已实现；持久化 branch summary entry、更完整 entry tree navigation UI、child branch navigation、跨 session parent/child entry graph 和 sidecar metadata 尚未实现。
...
- 当前只支持当前 session 文件内的指定 leaf entry path fork/clone、最小 entry-level branch、branch operation summary、entry path state derivation、entry tree active path 展示和 session-level parent navigation；`SessionManager` 仍维护 active `Message[]` 运行期缓存，entry-tree-first 内部主状态完全收敛、持久化 branch summary entry、完整 entry navigation UI、跨 session parent/child entry graph 与完整 child branch navigation 还未实现。
