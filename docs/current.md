...
当前 M4 已完成最小闭环：`SessionManager` 支持向后兼容的 lineage metadata、`forkSession()`、`cloneSession()`、`branchSession()`、`exportSession()`、`importSession()`、`listSessionTree()` 和旧 JSONL root fallback；RuntimeHost 暴露 `forkSession()`、`cloneSession()`、`branchSession()`、`exportSession()`、`importSession()` 和 `switchToParentSession()`；interactive/TUI 可通过 `/fork [id] [--entry <entryId>]` 创建当前 session 分支，也可通过 `/clone [id] [--entry <entryId>]` 按 `pi-mono` current-leaf fork 语义克隆当前 session；interactive `/branch <entryId>` 可在同 session 文件内移动 active leaf；RPC 已支持 `fork_session` / `clone_session` / `branch_session`；interactive/TUI 可通过 `/export [path]`、`/import <path>` 做 JSONL import/export；interactive `/sessions` 已展示 session tree，`/parent` 可切换到当前 session 的 parent session；新写入的 session entries 已带有 `entryId` / `parentEntryId`；`SessionManager.getEntryPath()` 可从 active entry leaf 回溯 path entries；`SessionManager.forkSession()` / `cloneSession()` 已优先复制指定 leaf entry path 到新 session；`SessionManager.branchSession()` 已从指定 leaf path 派生当前 active messages；`SessionContextRebuilder` 已支持新 session 的 `entry_path` rebuild，并对旧 JSONL 回退 `flat_snapshot`；`SessionManager.loadSession()` 已在主加载路径中使用 active entry path 重建 active messages。
...
- interactive mode 已实现 `/new`、`/resume`、`/resume <id>`、`/clear`、`/history`、`/stats`、`/diagnostics`、`/reload`、`/sessions`、`/parent` 和 `/branch`。
...
- M4 后续：entry-tree-first 对齐计划已明确，指定 leaf entry fork/clone 和 entry-level branch 最小步已实现；branch summary、更完整 entry tree navigation UI、child branch navigation、跨 session parent/child entry graph 和 sidecar metadata 尚未实现。

## 下一步

- 继续 M4：补 entry tree 展示中的 entry id 可见性和 branch navigation UX，让 `/branch <entryId>` 可发现、可操作。
- 也可补更完整的 child branch navigation，但需要避免只增强 session-level lineage，而不推进 entry-level leaf/path 语义。
...
- 当前只支持当前 session 文件内的指定 leaf entry path fork/clone、最小 entry-level branch 和 session-level parent navigation；`SessionManager` 仍维护 active `Message[]` 派生状态，entry-tree-first、branch summary、完整 entry navigation UI、跨 session parent/child entry graph 与完整 child branch navigation 还未实现。
