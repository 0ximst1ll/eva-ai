...
- session 管理继续完善：下一步应在 workspace store 之上继续拆单 session entry storage 和 session 语义层，减少 `SessionManager` 的 path traversal / state derivation 职责。

## 下一步

- 继续参考 pi-mono harness 的 `SessionStorage` / `SessionRepo` / `Session` 分层，拆出单 session entry storage 和 session 语义对象。
- 保持 `SessionManager` public API 不变，逐步把 entry index/path traversal 和 active leaf 操作移出单体类。
- 后续补完整 tree navigation 交互和 branch summarization pipeline。

## 已知问题

- `SessionManager` 仍是偏大的 facade，同时负责 entry path 状态、session lifecycle、fork/clone、import/export 和 lineage；workspace JSONL 文件 IO 已拆出，但尚未拆成完整 repo/storage/session 三层。
