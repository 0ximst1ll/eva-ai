...
- M4.x session reliability 仍处于收尾阶段；当前已完成 parse/load diagnostics、最小 schema validation、active path 不变量测试和 load/import fatal vs recoverable 策略。

## 下一步

- 优先保持现有 session tree 行为稳定，不在当前阶段继续拆 `SessionRepo`。
- 下一步继续补 manifest mismatch、schema migration 提示和更完整的 session recovery smoke case。
...
- session load/import 的 structured diagnostics、最小 schema validation、active path 不变量测试和 fatal vs recoverable 策略已有最小闭环；manifest mismatch、schema migration 提示和更完整 recovery smoke case 仍需继续补齐。
