...
- ToolResult `contentBlocks + durable typed details` 最小闭环已实现：tool result 已支持 text block content，并可随 session reload/rebuild 保留；agent-loop 会透传并持久化工具结构化 details，`read`、`bash`、`grep`、`find`、`ls` 已输出工具专属 details 类型，包含 truncation、行数/结果数、exit code、full output path 等结构化信息；provider request 转换会把 block content flatten 为文本，并剥离 durable details，只发送 tool result 文本。
...
- Eva 已有 metadata、governance hook、read-only 并发、write/bash 串行、runtime schema validation、`renderCall`、`renderResult`、text block content、durable typed details 和 partial update。
...
- 第五优先级：Tool System 补 image block content、rich renderer/export renderer 和 extension tool registry 边界。
...
- Tool Result 已有 `contentBlocks + durable typed details` 和工具级 `renderResult` 最小边界；text blocks 和 details 已能随 session reload/rebuild 保留，但 image blocks、rich renderer/export renderer 仍未完成。
