...
- M4 后续：entry-tree-first 对齐计划已明确，指定 leaf entry fork/clone、entry-level branch、entry tree active path 展示和 branch operation summary 最小步已实现；持久化 branch summary entry、更完整 entry tree navigation UI、child branch navigation、跨 session parent/child entry graph 和 sidecar metadata 尚未实现。

## 下一步

- 继续 M4：可补更完整的 entry tree navigation UI / TUI selector，让 entry-level branch 更接近可长期使用的 session tree 操作。
- 也可推进 entry-tree-first 内部状态收敛，把 active `Message[]` 进一步降级为从 active leaf path 派生出的 context view。
